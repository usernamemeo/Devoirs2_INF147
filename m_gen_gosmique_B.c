...
t_Gen_GosmiqueB* creer_gen_gosmique_b(int nb_lignes, double p_transit_vide) {
    ...
    t_Gen_GosmiqueB* gen = malloc(sizeof(t_Gen_GosmiqueB));
    ...
    gen->grille_bits = malloc(gen->taille_tableau * sizeof(uchar));
...
int get_etat_capteur_B(t_Gen_GosmiqueB* gen, int ligne, int colonne) {
    ...
    // Lire bit0
    bit0 = get_bit_char(gen->grille_bits[byte_index], bit_offset);

    // Lire bit1 (peut être dans le byte suivant)
    if (bit_offset == BITS_CHAR - 1) {
        // bit1 est dans le byte suivant
        if (byte_index + 1 < gen->taille_tableau) {
            bit1 = get_bit_char(gen->grille_bits[byte_index + 1], 0);
        } else {
            bit1 = 0;
        }
    } else {
        // bit1 est dans le même byte
        bit1 = get_bit_char(gen->grille_bits[byte_index], bit_offset + 1);
    }
...
int set_etat_capteur_B(t_Gen_GosmiqueB* gen, int ligne, int colonne, int nouvel_etat) {
    ...
    // Écrire bit1 (peut être dans le byte suivant)
    if (bit_offset == BITS_CHAR - 1) {
        // bit1 va dans le byte suivant
        if (byte_index + 1 < gen->taille_tableau) {
            if (bit1) {
                gen->grille_bits[byte_index + 1] = set_bit_char(gen->grille_bits[byte_index + 1], 0);
            } else {
                gen->grille_bits[byte_index + 1] = clear_bit_char(gen->grille_bits[byte_index + 1], 0);
            }
        }
    } else {
        // bit1 est dans le même byte
...
int prochain_capteur_B(t_Gen_GosmiqueB* gen) {
    ...
    typedef struct { int ligne; int colonne; } Position;
    ...
    for (int i = 0; i < nb_lignes_possibles; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (nouvelle_colonne >= 0 && nouvelle_colonne < NB_COLONNES &&
    ...
    int sort_du_gen = 0;
    if (peut_sortir && nb_candidats > 0 && RANDF00() < 0.05)
        sort_du_gen = 1;
    ...
    if (nb_candidats == 0 || sort_du_gen) {
        ...
        gen->somme_carres_longueurs += (long long)(gen->longueur_parcours_actuel) * gen->longueur_parcours_actuel;
        return 0;
    }
